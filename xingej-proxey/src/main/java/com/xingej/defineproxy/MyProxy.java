...
public class MyProxy {
    ...
    public static Object createProxyInstance(ClassLoader loader, Class interf, MyInvocationHandler h) throws Exception {
        ...
        String proxyClassString = "package com.xingej.defineproxy;" + rt 
                + "import java.lang.reflect.Method;" + rt
                + "public class $Proxy4 implements "
                + interf.getName() //得到是Person
                + "{" +  rt
                + "MyInvocationHandler h;" + rt
                +"public $Proxy4(MyInvocationHandler h) {" + rt
                +"this.h = h;" + rt
                +"}" + rt
                + getMethodString(methods, interf)
                + rt
                + "};";
